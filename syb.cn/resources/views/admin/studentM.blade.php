                ...
                @if(isset($students))
                    <table class="table table-striped table-bordered table-hover table-condensed" id="main-table">
                        ...
                        @for($i=0;$i<count($students);$i++)
                            <tr id="{{$students[$i]->student_id}}">
                                <td>{{$i + 1}}</td>
                                <td class="student_id">{{$students[$i]->student_id}}</td>
                                <td>{{$students[$i]->student_name}}</td>
                                <td>{{$students[$i]->email}}</td>
                                <td>{{$students[$i]->password}}</td>
                                <td>{{$students[$i]->register_at}}</td>
                                <td class="action">
                                    <button class="btn btn-primary actionBtn"  data-toggle="modal" data-target="#sendModal" data-id="{{$students[$i]->student_id}}" data-name="{{$students[$i]->student_name}}">发送信息</button>
                                    {{--<button class="btn btn-danger" data-toggle="modal" data-target="#delModal" data-id="{{$students[$i]->student_id}}" data-name="{{$students[$i]->student_name}}">删除</button>--}}
...
//        发送消息给学员
        ...
        $('.sendmsg').click(function () {
            ...
            $.post('sendmsg/student_id/'+student_id,{'_token':"{{csrf_token()}}",'title':title,'content':content},function (res) {
                if(res === 'y'){
...
//        学员搜索
        $('.search').click(function () {
            ...
            $.post('search_student/'+search_key,{'_token':"{{csrf_token()}}"},function(data){
...
//        点击删除操作时弹出模态框，确认删除时调用AJAX函数
        ...
        $('.del-ok').click(function () {
            let student_id = $(this).attr('data-id')
            $.post('delstudent/student_id/'+student_id,{'_token':"{{csrf_token()}}"},function (res) {
                if(res === 'y'){
