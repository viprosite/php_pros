...
@section('content')
    <div class="container newsbox center-block" style="width: 60%;">
            ...
            @if(count($res) != 0)
                <ul class="list-group">
                    @for($i=0;$i<count($res);$i++)
                        <li class="list-group-item" id="{{$res[$i]->id}}">
                                @if($res[$i]->viewor == 1)
                                <a href="{{url('msgdetail/'.$res[$i]->id)}}" target="">
                                    <span class="text-success"><i class="fa fa-check-square-o"></i> 已读</span>
                                    <span> <mark>{{$res[$i]->sender}}</mark> 发送于：{{$res[$i]->send_time}}</span> &nbsp;&nbsp;
                                    <b>{{$res[$i]->title}}</b>
                                </a> &nbsp;&nbsp;&nbsp;
                                <button class="btn btn-sm btn-danger delmsgbtn" data-toggle="modal" data-target="#delModal" data-id="{{$res[$i]->id}}">删除</button>
                                @else
                                <a href="{{url('msgdetail/'.$res[$i]->id)}}" target="">
